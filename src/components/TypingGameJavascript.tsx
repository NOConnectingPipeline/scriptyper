...
const randomWordJavascript: string[] = [];
const randomMeanJavascript: string[] = [];
const randomLevelJavascript: number[] = [];

const max = 10

const lengthJSONJs: number = WordContainment.js.length;
...
for(let i = 0; i < max; i++){
  while(true) {
    let tmp = numRandom(lengthJSONJs)
    if(!randomWordJavascript.includes(WordContainment.js[tmp].word)){
      randomWordJavascript.push(WordContainment.js[tmp].word);
      randomMeanJavascript.push(WordContainment.js[tmp].mean);
      randomLevelJavascript.push(WordContainment.js[tmp].level);
      break;
    }
  }
}
...
const TypingGameJavascript = () => {
  ...
  let chars: string = randomWordJavascript[intRandom]
  ...
  const changeTyping = () => {
    ...
    chars = randomWordJavascript[intRandom];
    console.log(chars)
    resetTyping();
    console.log(charsState);
  }
  ...
  const judgeTyping = () => {
    ...
    if(!charsState.includes(2)) {
      tmpCalculatingScore.push(randomLevelJavascript[intRandom])
  ...
  return (
    ...
    <p>LEVEL: {randomLevelJavascript[intRandom]}</p>
    <p>{randomMeanJavascript[intRandom]}</p>
    </>
  );
  }
};
export default TypingGameJavascript
;