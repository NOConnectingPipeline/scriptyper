...
const randomWordGit: string[] = [];
const randomMeanGit: string[] = [];
const randomLevelGit: number[] = [];

const max = 10

const lengthJSONGit: number = WordContainment.git.length;
...
for(let i = 0; i < max; i++){
  while(true) {
    let tmp = numRandom(lengthJSONGit)
    if(!randomWordGit.includes(WordContainment.git[tmp].word)){
      randomWordGit.push(WordContainment.git[tmp].word);
      randomMeanGit.push(WordContainment.git[tmp].mean);
      randomLevelGit.push(WordContainment.git[tmp].level);
      break;
    }
  }
}
...
const TypingGameGit = () => {
  ...
  let chars: string = randomWordGit[intRandom]
  ...
  const judgeTyping = () => {
    ...
    if(!charsState.includes(2)) {
      tmpCalculatingScore.push(randomLevelGit[intRandom])
  ...
  return (
    ...
    <p>LEVEL: {randomLevelGit[intRandom]}</p>
    <p>{randomMeanGit[intRandom]}</p>
    </>
  );
  }
};
export default TypingGameGit
;